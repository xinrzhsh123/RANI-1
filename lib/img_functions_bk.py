...
def process_image(channel_img, win_x, win_y, std = False, remove_zeros = False):
	...
	if remove_zeros:
		...
		return channel_img
               ...
               if np.max(channel_img)>255:
                   Num_bins=65536
                   bins=xrange(Num_bins+1)
                   histo=np.histogram(channel_img,bins=bins)
                   histI=histo[0]
                   mat_histI = matlab.double(histI.tolist())
                   eng.addpath(r'lib',nargout=0)
                   pTri=eng.triangle_th2(mat_histI,Num_bins)
               elif np.max(channel_img)>1:
                   Num_bins=256
                   bins=xrange(Num_bins+1)
                   histo=np.histogram(channel_img,bins=bins)
                   histI=histo[0]
                   ...
                   #Num_bins=256
                   #histo=np.histogram(channel_img,bins=Num_bins,range=(0.0,1.0))
                   #histI=histo[0]
                   #mat_histI = matlab.double(histI.tolist())
                   #eng.addpath(r'lib',nargout=0)
                   #pTri=eng.triangle_th2(mat_histI,Num_bins)
...
def train_model_sample(model = None, dataset = None,  optimizer = None, 
	...
	rotate = True, flip = True, shear = 0, class_weight = None, todays_date=None):
...
def run_model(image, model, win_x = 30, win_y = 30, std = False, split = True, process = False):
...
def run_models_on_directory(data_location, channel_names, output_location, model_fn, list_of_weights, n_features = 3, image_size_x = 1080, image_size_y = 1280, win_x = 30, win_y = 30, std = False, split = True, process = True, save = True, nuclear_prefix='', NumReferImg=0, Nextcascade= False):
        #pdb.set_trace() # QQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQ
	...
	for weights_path in list_of_weights:
		...
		processed_image= run_model(image, model, win_x = win_x, win_y = win_y, std = std, split = split)
...
def run_models_on_lsm(lsm_file, output_location, model_fn, list_of_weights, n_features = 3, image_size_x = 512, image_size_y = 512, win_x = 15, win_y = 15, std = False, split = True, save = True):
	...
	if save:
		for img in xrange(model_output.shape[0]):
			for feat in xrange(n_features):
				feature = model_output[img,feat,:,:]
                                cnnout_name = os.path.join(output_location,  'feature_' + str(feat) + '_' + img_files[0][img][0:-4] + r'.tif')
				tiff.imsave(cnnout_name,feature)
...
def segment_nuclei(img = None, save = True, adaptive = False, color_image = False, load_from_direc = None, feature_to_load = "feature_1", mask_location = None, threshold = 0.5, area_threshold = 50, eccentricity_threshold = 1, solidity_threshold = 0, nuclear_prefix=''):
	# Requires a 4 channel image (number of frames, number of features, image width, image height)
	from skimage.filters import threshold_otsu, threshold_adaptive
        img_files = []                                                           #QQQQQQ
        img_files = nikon_getfiles(load_from_direc, feature_to_load )            #QQQQQQ
	if img is not None:                                                          #QQQQQQ
		img = img[:,1,:,:]
	...
	for frame in xrange(img.shape[0]):
		...
		nuclear_masks[frame,:,:] = nuclear_mask
                ...
                #pdb.set_trace() # QQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQ
		if save:
			img_name = os.path.join(mask_location, "mask_" + img_files[frame][0:-4] + "_"+ nuclear_prefix +".png")#QQQQQQ
